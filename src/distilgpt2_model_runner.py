...
# Local fine-tuned model folder created by train_distilgpt2.py
MODEL_DIRECTORY = (
    PROJECT_FOLDER
    / "models"
    / "distilgpt2_continuation"
    / CONDITION
)
...
def main():
    ...
    # Loads the fine-tuned model saved by the training script

    if not MODEL_DIRECTORY.exists():
        raise FileNotFoundError(
            "The fine-tuned DistilGPT-2 folder was not found: "
            f"{MODEL_DIRECTORY}\n"
            "Run src/train_distilgpt2.py first so the model weights are saved."
        )

    print("Loading the tokenizer from the local fine-tuned model...")
    ...
    print("Loading fine-tuned DistilGPT-2 model...")
